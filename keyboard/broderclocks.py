...
class BroderClocks:
    ...
    def __init__(self, parent, canvas, centers, win_diffs, radius, file_handle, clocks_on, clocks_off, circle_off_color, in_time, use_num, user_id, time_rotate, prev_data):
        ### process the inputs ###
        ## copy
        self.parent = parent
        self.canvas = canvas
    ...
    def select(self, time_in):
	    ...
	    self.update_bits()
            ...
            (self.clocks_on, self.clocks_off, clock_score_prior, self.is_undo, self.is_equalize) = self.parent.make_choice(self.sorted_inds[0])
            # learn new scores
            if config.is_learning:
                self.learn_scores()
    ...
    def update_scores(self, time_diff_in):
        ...
        for clock in self.clocks_on:
	    time_in = self.cur_hours[clock]*self.time_rotate*1.0/self.num_divs_time + time_diff_in - self.time_rotate*config.frac_period
    ...
    def update_history(self, time_diff_in):
        ...
        while i_all < self.N_clocks:
            ...
            if clock == i_all:
		click_time = (self.cur_hours[clock]*self.time_rotate*1.0/self.num_divs_time + time_diff_in - self.time_rotate*config.frac_period + 0.5)%1 - 0.5 # mod and 0.5's for keeping in (-0.5,0.5) range
